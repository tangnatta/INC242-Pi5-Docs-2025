...
16. After successfully logged in to your Raspberry Pi 5, start updating and upgrading the raspberry pi by running the following commands:
    ...
    sudo apt upgrade -y
    ```
17. (Optional, but recommended) Install Desktop for easier usage
    ```
    sudo apt install ubuntu-desktop -y
    ```
<!-- 18. Install desktop manager `sudo apt install lightdm -y`
19. Select lightdm as the default display manager when prompted -->
20. Set Raspberry pi 5 to boot into desktop environment by type `sudo systemctl set-default graphical.target`
21. Reboot the Raspberry Pi 5 to apply all changes
    ```
    sudo reboot
    ```
22. You can now use your Raspberry Pi 5 for your projects!
